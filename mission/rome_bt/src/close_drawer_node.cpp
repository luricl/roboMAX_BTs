#include "close_drawer_node.h"


closeDrawerNode::closeDrawerNode(const std::string &xml_tag_name,
                             const BT::NodeConfiguration &conf,
                             rclcpp::Node::SharedPtr node_ptr): BT::ConditionNode(xml_tag_name, conf){
    std::string topic = "close_drawer";

    auto qos = rclcpp::SystemDefaultsQoS();
    qos.best_effort();
    str_subscriber_ptr_ = node_ptr->create_subscription<std_msgs::msg::String>(topic, qos,
                                                                               std::bind(&closeDrawerNode::update_msg, this, std::placeholders::_1));
}

void closeDrawerNode::update_msg(const std_msgs::msg::String::SharedPtr new_msg){
    msg_ = new_msg->data;
}

// Waits for given duration or until message is received
bool closeDrawerNode::wait_for_message(int wait_duration){
...
BT::NodeStatus closeDrawerNode::tick(){
