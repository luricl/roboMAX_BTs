#include "wait_for_deposit_node.h"


waitForDepositNode::waitForDepositNode(const std::string &xml_tag_name,
    ...
    str_subscriber_ptr_ = node_ptr->create_subscription<std_msgs::msg::String>(topic, qos,
                                                                               std::bind(&waitForDepositNode::update_msg, this, std::placeholders::_1));
}

void waitForDepositNode::update_msg(const std_msgs::msg::String::SharedPtr new_msg){
    msg_ = new_msg->data;
}

// Waits for given duration or until message is received
bool waitForDepositNode::wait_for_message(int wait_duration){
...
BT::NodeStatus waitForDepositNode::tick(){
