#include "pickup_sample_node.h"


pickupSampleNode::pickupSampleNode(const std::string &xml_tag_name,
                             const BT::NodeConfiguration &conf,
                             rclcpp::Node::SharedPtr node_ptr): BT::ConditionNode(xml_tag_name, conf){
    std::string topic = "pickup";

    auto qos = rclcpp::SystemDefaultsQoS();
    qos.best_effort();
    str_subscriber_ptr_ = node_ptr->create_subscription<std_msgs::msg::String>(topic, qos,
                                                                               std::bind(&pickupSampleNode::update_msg, this, std::placeholders::_1));
}

void pickupSampleNode::update_msg(const std_msgs::msg::String::SharedPtr new_msg){
    msg_ = new_msg->data;
}

// Waits for given duration or until message is received
bool pickupSampleNode::wait_for_message(int wait_duration){
    rclcpp::WaitSet wait_set;
    wait_set.add_subscription(str_subscriber_ptr_);
    RCLCPP_INFO_STREAM(node_ptr_->get_logger(), "waiting pickup signal...\n");
...
BT::NodeStatus pickupSampleNode::tick(){
