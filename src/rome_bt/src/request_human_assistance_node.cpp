#include "request_human_assistance_node.h"


requestHumanAssistanceNode::requestHumanAssistanceNode(const std::string &xml_tag_name,
                             const BT::NodeConfiguration &conf,
                             rclcpp::Node::SharedPtr node_ptr): BT::SyncActionNode(xml_tag_name, conf)
{
    
    std::string topic = "assist";
    auto qos = rclcpp::SystemDefaultsQoS();
    qos.best_effort();
    str_publisher_ptr_ = node_ptr->create_publisher<std_msgs::msg::String>(topic, qos);
}

BT::NodeStatus requestHumanAssistanceNode::tick(){
