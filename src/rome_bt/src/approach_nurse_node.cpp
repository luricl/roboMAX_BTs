#include "approach_nurse_node.h"


approachNurseNode::approachNurseNode(const std::string &xml_tag_name,
                             const BT::NodeConfiguration &conf,
                             rclcpp::Node::SharedPtr node_ptr): BT::ConditionNode(xml_tag_name, conf), node_ptr_(node_ptr)
{
    RCLCPP_DEBUG(node_ptr->get_logger(), "Achieved here");
    
    std::string topic = "nurses";

    auto qos = rclcpp::SystemDefaultsQoS();
    qos.best_effort();
    str_subscriber_ptr_ = node_ptr->create_subscription<std_msgs::msg::String>(topic, qos,
                                                                               std::bind(&approachNurseNode::update_msg, this, std::placeholders::_1));
}

void approachNurseNode::update_msg(const std_msgs::msg::String::SharedPtr new_msg){
    msg_ = new_msg->data;
}

// Waits for given duration or until message is received
bool approachNurseNode::wait_for_message(int wait_duration){
...
BT::NodeStatus approachNurseNode::tick(){
    ...
    RCLCPP_INFO_STREAM(node_ptr_->get_logger(), "approaching nurse: " << msg_.c_str() << "\n");
    setOutput("nurse", msg_);
